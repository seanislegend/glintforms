...
const ExportResponsesFiltersControls: React.FC<Props> = ({filters}) => {
    ...
    return (
        <div className="grid gap-3">
            <FormField
                control={control}
                description="Apply the active filters to the export."
                fieldType="switch"
                label="Apply active filters"
                name="applyActiveFilters"
            />
            <ToggleVisibility visible={applyActiveFilters}>
                <BasicCard
                    description="The active filters will be applied to the export. If you want to remove or apply different filters, you can do so by changing the filters in the table."

...
program
    ...
    .option('-c, --config <path>', 'Path to config file', '.translation.config.json')
    .option('-a, --app <name>', 'Extract for specific app only')
    ...
    .action(async options => {
        try {
            ...
            if (!options.locale && !options.all) {
                console.log('Please specify --locale <locale> or --all to check all locales');
                process.exit(3);
            }

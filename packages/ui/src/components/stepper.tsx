...
const StepperTrigger = ({className, children, tabIndex, ...props}: StepperTriggerProps) => {
...
const StepperIndicator = ({children, className}: React.ComponentProps<'div'>) => {
    const {state, isLoading} = useStepItem();
    ...
    return (
        <div
            ...
            data-state={state}
            className={cn(
                'relative flex items-center overflow-hidden justify-center size-6 shrink-0 border-background bg-accent text-accent-foreground rounded-full text-xs data-[completed]:bg-primary data-[completed]:text-primary-foreground data-[active]:bg-primary data-[active]:text-primary-foreground',
                className
            )}
        >
...
const StepperSeparator = ({className}: React.ComponentProps<'div'>) => {
...
const StepperTitle = ({children, className}: React.ComponentProps<'h3'>) => {
    const {state} = useStepItem();
...
const StepperDescription = ({children, className}: React.ComponentProps<'div'>) => {
    const {state} = useStepItem();
...
const StepperNav = ({children, className}: React.ComponentProps<'nav'>) => {
...
const StepperPanel = ({children, className}: React.ComponentProps<'div'>) => {
    const {activeStep} = useStepper();

    return (
        <div data-slot="stepper-panel" data-active={activeStep} className={cn('w-full', className)}>
            {children}
        </div>
    );
};
...
const StepperContent = ({value, forceMount, children, className}: StepperContentProps) => {
    const {activeStep} = useStepper();
